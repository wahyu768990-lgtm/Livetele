...
def get_drive_direct_url(video_url):
    ...
    if drive_match:
        ...
        url_with_params = action_url + "?" + "&".join([f"{k}={v}" for k, v in params.items()])

        return url_with_params, session.cookies.get_dict(), session

    return video_url, {}, session

url, cookies, session = get_drive_direct_url("https://drive.google.com/uc?id=1ddFW6aV5dJxhV2wdfmEFf4K48d4G9MFA&export=download")

print("Final URL:", url)
print("Cookies:", cookies)

headers = {"Cookie": "; ".join([f"{k}={v}" for k, v in cookies.items()])}
print("Headers for aria2/ffmpeg:", headers)
